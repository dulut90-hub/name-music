...
export default function App() {
  ...
  const handleSearch = async (e?: React.FormEvent, directQuery?: string) => {
    ...
    try {
      ...
      if (list.length > 0) {
        ...
        const ytResponse = await fetch("/api/ytplay", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query: searchQuery }) });
        const ytData = await ytResponse.json(); if (ytData.status && ytData.result) setResults([ytData.result]);
      }
  ...
  const fetchDownloadLinks = async (video: VideoResult) => {
    ...
    try {
      const response = await fetch("/api/ytplay", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query: video.url }) });
      const data = await response.json();
